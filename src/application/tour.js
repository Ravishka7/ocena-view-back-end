...
export const createTour = (req, res) => {
  const tour = req.body;
    ...
    if (!tour.image || !tour.name || !tour.location || !tour.price) {
...
export const deleteTour = (req, res) => {
  ...
  if (!tours.find((tour) => tour._id === tourId)) {
    return res.status(404).json();
  }

  tours.splice(tours.findIndex((tour) => tour._id === tourId), 1);
...
export const updateTour = (req, res) => {
  ...
  tours[tours.findIndex((tour) => tour._id === tourId)] = {
    _id: tourId,
    image: tour.image,
    name: tour.name,
    location: tour.location,
